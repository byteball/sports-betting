...
describe('Define AAs test', function () {
	...
	before(async () => {
		...
		.with.agent({ registry: 	path.join(__dirname, '../registry/sl_token_registry.ojson') })
	...
	it('alice defines and issues assets issuer_0', async () => {
		...
		expect(alice_balances[vars["hometeam"]].stable).to.be.equal(alice_base_amount_paid - 10000);
		expect(alice_balances[vars["awayteam"]].stable).to.be.equal(alice_base_amount_paid - 10000);
		expect(alice_balances[vars["draw"]].stable).to.be.equal(alice_base_amount_paid - 10000);
		expect(alice_balances[vars["canceled"]].stable).to.be.equal(alice_base_amount_paid - 10000);
		...
		var hometeam_symbol =  this.issuer_0_feed_name + '-' + this.issuer_0_hometeam + '-GBYTE';
		...
		expect(vars['decimals_' + this.hometeam_asset +  right_part]).to.be.equal(9)

		var awayteam_symbol =  this.issuer_0_feed_name + '-' + this.issuer_0_awayteam + '-GBYTE';
		...
		expect(vars['decimals_' + this.awayteam_asset +  right_part]).to.be.equal(9)

		var draw_symbol =  this.issuer_0_feed_name + '-DRAW-GBYTE';
		...
		expect(vars['decimals_' + this.draw_asset +  right_part]).to.be.equal(9)

		var canceled_symbol =  this.issuer_0_feed_name + '-CANCELED-GBYTE';
		...
		expect(vars['decimals_' + this.canceled_asset +  right_part]).to.be.equal(9)
	})
	...
	it('bob defines and issues assets issuer_1', async () => {
		...
		var hometeam_symbol =  this.issuer_1_feed_name + '-' + this.issuer_1_hometeam + '-GBYTE';
		expect(vars['s2a_' + hometeam_symbol + right_part]).to.be.equal(this.hometeam_asset)
		expect(vars['a2s_' + this.hometeam_asset +  right_part]).to.be.equal(hometeam_symbol)

		var awayteam_symbol =  this.issuer_1_feed_name + '-' + this.issuer_1_awayteam + '-GBYTE';
		expect(vars['s2a_' + awayteam_symbol + right_part]).to.be.equal(this.awayteam_asset)
		expect(vars['a2s_' + this.awayteam_asset +  right_part]).to.be.equal(awayteam_symbol)

		var draw_symbol =  this.issuer_1_feed_name + '-DRAW-GBYTE';
		expect(vars['s2a_' + draw_symbol + right_part]).to.be.equal(this.draw_asset)
		expect(vars['a2s_' + this.draw_asset +  right_part]).to.be.equal(draw_symbol)

		var canceled_symbol =  this.issuer_1_feed_name + '-CANCELED-GBYTE';
		expect(vars['s2a_' + canceled_symbol + right_part]).to.be.equal(this.canceled_asset)
		expect(vars['a2s_' + this.canceled_asset +  right_part]).to.be.equal(canceled_symbol)
